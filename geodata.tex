...
\newcommand{\sub}[1][2]{#1\textsuperscript{#2}}
...
\begin{document}
	...
	\begin{enumerate} 
		...
		\item Spatial join processing cannot exploit the technique of data declustering which is generally used as the basis for processing natural joins in parallel (partitioned parallelism [Gra93]). Given a declustered data placement of spatial relations R and S into p disjoint subsets $R_1$,...,$R_n$ and $S_1$,...,$S_n$, respectively, the union of the response sets obtained from processing spatial joins of $R_j$ and $S_j$, 1 ≤ j ≤ p, is only a subset of the response set of the spatial join of R and S.
	\end{enumerate} 
	...
	\citep{Orenstein}. The \term{filter step} during which all objects that 
	...
	those objects really fulfill the query condition.
	\\



	\citep{Brinkhoff93}:
	Let us consider two sets A={$a_1$,...,$a_n$} and B={$b_1$,...,$b_m$} of spatial objects, known as spatial relations. The reader may think of the objects in a spatial relation as polygons representing a surface of a spatial object. Let Id be a function that assigns a unique identifier to each spatial object in a database. Furthermore, let \acr{mbr} be a function that computes the minimum bounding rectilinear rectangle of an object. For example, \acr{id}($a_1$) and \acr{mbr}($a_1$) denote the identifier and the minimum bounding rectangle of $a_1$, respectively. The most important spatial joins are the following:\\
	1) \acr{mbr}-spatial-join: Compute all pairs (\acr{id}(\sub{a,i}), \acr{id}(\sub{b,j})) with \acr{mbr}(\sub{a,i}) \cap \acr{mbr}(\sub{b,j}) \ne $\oldemptyset$\\
	\acr{mbr}-spatial-join: Compute all pairs $(\acr{id}(a_i), \acr{id}(b_j))$ with $\acr{mbr}(a_i) \cap \acr{mbr}(b_j) \ne \oldemptyset$\\
	2) \acr{id}-spatial-join: Compute all pairs (\acr{id}(\sub{a,i}), \acr{id}(\sub{b,j})) with \sub{a,i} \cap \sub{b,j} \neq $\emptyset$\\
	\acr{id}-spatial-join: Compute all pairs $(\acr{id}(a_i), \acr{id}(b_j))$ with $a_i \cap b_j \neq \emptyset$\\
	3) Object-spatial-join: Compute \sub{a,i} \cap \sub{b,j} with \sub{a,i} \cap \sub{b,j} \neq $\emptyset$\\
	Object-spatial-join: Compute $a_i \cap b_j$ with $a_i \cap b_j \neq \emptyset$
	\par
	...
	Spatial joins are distinguished from a standard relational join \citep{Mishra} in that the join condition involves the multi-dimensional spatial attribute of the joined relation.
	\\

	There exist several methods for performing a spatial join on R-trees \citep{Guttman}.
	\\

	Applications such as \acr{gis} require the ability to deal with data objects other than points and query objects other than boxes \citep{Orenstein}. This motivates 
	...
	1) Orenstein[18] has proposed B+-trees in combination with z-ordering as the underlying access method for performing spatial joins.
	\\
