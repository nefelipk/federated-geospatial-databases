...
\begin{document}
	...
	\citet{Jacox} comprehensive survey
	\\
	...
	Typically, a spatial join is performed in two stages: the \term{filter} stage in which complicated polygonal objects are approximated by rectangles and the \term{refinement} stage which removes any results produced during the filtering stage that do not satisfy the join condition \citep{Jacox}. Filter-and-refine approach [Orenstein 1989b].
	\\
	...
	2) Vectors (a list of vertices) are commonly used to store polygons, but rasterapproaches are also used [Orenstein 1986].
	\\
