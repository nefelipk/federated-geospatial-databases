...
\begin{document}
	...
	example, a two-level grid file is shown in 	Figure~\bl{\ref{fig:mlgf}}, 
	...
	given in Figures~\bl{\ref{fig:mlgf}}.
	\\

	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.5\textwidth]{mlgf}
		\caption{Multilevel grid file structure (Figure 3 \citep{Whang}).}
		\label{fig:mlgf:temp}
	\end{figure}

																							\todo[size={{\scriptsize}}]{same d1 and d2 in Kim and Whang ???}
	\begin{figure}[h!]
		\centering
		\begin{subfigure}[t]{0.5\textwidth}
			\includegraphics[width=\textwidth]{mlgf}
			\caption{The structure of a two-level grid file (Figure 3 \citep{Whang}).}
		\end{subfigure}
		\\
		\begin{subfigure}[t]{0.4\textwidth}
			\includegraphics[width=\textwidth]{mlgf_d1}
			\caption{Regions represented by directory entries in $D\textsubscript{1}$ (Figure 3b \citep{Kim}).}
		\end{subfigure}
		\hfill
		\begin{subfigure}[t]{0.4\textwidth}
			\includegraphics[width=\textwidth]{mlgf_d2}
			\caption{Regions represented by directory entries in $D\textsubscript{2}$ (Figure 3c \citep{Kim}).}
		\end{subfigure}
		\caption{Multilevel grid file structure.}
		\label{fig:mlgf}
	\end{figure}
...
Since many spatial indices are hierarchical, a spatial join algorithm for these indices also has a hierarchical nature \citep{Jacox}.
\todo[size={{\scriptsize}}]{hierarchical and non-hierarchical methods}
Any generalization tree structure can be applied efficiently to compute spatial joins in a hierarchical manner.
\todo[size={{\scriptsize}}]{hierarchical manner: differences between Gunther [1993], Brinkhoff [1993], Kim [1995], and Huang [1997b]}
If both datasets are indexed using generalization trees and both indices have the same height, then a spatial join can be performed efficiently with a \term{synchronized traversal} of the indices \citep{Jacox}. In the following we assume that the data objects are only stored in the leaves of the tree. Starting with the two root nodes of the indices, $rootA$ and $rootB$, the algorithm finds intersections between the sub-nodes of $rootA$ and $rootB$. The intersecting sub-node pairs are added to a priority queue [Cormen et al. 1990]. The pairs in the priority queue are checked for intersecting sub-nodes in later iterations. If the two nodes are leaves, they are compared and report any intersecting objects.
\\



For indices of different heights, the join of a leaf of one index with a sub-tree of the other can be accomplished using a window query \citep{Brinkhoff93}, or handling leaf-to-node comparison as special cases [Kim et al. 1995]. The idea of \citet{Brinkhoff93} is to perform window queries on the subtrees of the shorter tree using the data rectangles of the higher tree as query rectangles.
\\
...
The three (three ????????????????????????) 
variations of the synchronized traversal algorithm differ in the priority (i.e., the sort order) that is used by the priority queue. Each variation attempts to minimize disk accesses.
\\

\citet{Brinkhoff93} and \citet{Kim} deal with R-trees and multilevel grid files respectively. They both use a depth-first approach in which all of the sub-node pairs for a given node pair are processed before proceeding to the next node pair at the same level. To improve performance, \citet{Brinkhoff93} use two buffers. A path buffer is used to accommodate all of the ancestor nodes of the current node pair. And an additional \acr{lru} (\term{last recently used}) buffer is used for single pages, not complete paths, independently of the path buffer. The reason for two different buffers is that the path buffer exclusively belongs to the data structure, whereas the \acr{lru} buffer can be considered as a buffer of the underlying system.
\\
