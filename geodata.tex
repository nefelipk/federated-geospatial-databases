...
\begin{document}
	...
	\label{sec:geo:filter}

	During the filtering stage, a spatial join is performed on approximations 
	of the objects \citep{Jacox}. Let us start with techniques for performing 
	a spatial join without using external memory. That is no data is written 
	to external memory, only read once if necessary. 
	\\


	\subsubsection{Internal memory techniques}
	\label{sec:geo:filter:in}

	\paragraph{Plane sweep method}
	\label{sec:geo:filter:in:sweep}
	...
	\paragraph{Z-order method}
	\label{sec:geo:filter:in:z}
	...
	\paragraph{Index nested-loop join}
	\label{sec:geo:filter:in:nested}
	...
	\subsubsection{External memory techniques}
	\label{sec:geo:filter:ex}
	...
	memory techniques mentioned in Section~\ref{sec:geo:filter:in} (i.e. 
	...
	\paragraph{Spatial access methods}
	\label{sec:geo:filter:ex:access}
	...
	several common spatial access methods, such as R-trees.
	\\
	...
	contribution to the development of balanced multikey file structures. 
	...
	of data to be stored. \term{Multikey access} means that the reference of 
	records is done by using any possible subset of its fields, as shown in 
	the following examples: 
	\begin{itemize} 
		\item Entire record specified (exact match query, point query)
		\item Doe born in 1951 (a partially specified query)
		\item All records with last name Doe (single-key query) 
		\item Social security number 987654321 (presumably unique) 
		\item Everybody born between 1940 and 1960 (range or interval query) 
	\end{itemize} 
	The grid file is a point access method based on grid partitions of the 
	search space illustrated in Figure~\bl{\ref{fig:grid_file}}. Each region 
	boundary cuts the entire search space in two, so all dimensions are 
	treated symmetrically. Each partition is known as a cell of the grid. And 
	each cell contains a pointer to the appropriate bucket in the database 
	where the actual data (i.e. the record) is stored. The grid file provides 
	...
	\paragraph{Both datasets indexed}
	\label{sec:geo:filter:ex:both}
	...
	techniques from Section~\ref{sec:geo:filter:in} should be used 
	\citep{Jacox}. For smaller page sizes, a nested-loop join 
	(Section~\ref{sec:geo:filter:in:nested}) might be best because of the 
	low overhead. For larger page sizes, the plane-sweep method 
	(Section~\ref{sec:geo:filter:in:sweep}), or a Z-order sweep 
	(Section~\ref{sec:geo:filter:in:z}) would be more appropriate.
	\\
...
\paragraph{One dataset indexed, one not}
\label{sec:geo:filter:ex:both}

\paragraph{Neither dataset indexed}
\label{sec:geo:filter:ex:both}
