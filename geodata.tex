...
\begin{document}
	...
	Typically, a spatial join is performed in two steps, the \term{filter step} and the \term{refinement step} \citep{Orenstein}. During the filter step the objects that could not possibly satisfy the query are rapidly eliminated. The result of this step is a set of \term{candidate objects} which includes all the objects satisfying the query, and possibly some false hits. During the refinement step each candidate is examined and false hits are detected and eliminated. In extreme cases, the filter step would return the entire set of data objects, all of which would then have to be considered by the refinement step.
	\\
