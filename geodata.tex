...
\begin{document}
	...
	\begin{itemize} 
		\item \acr{mbr}-spatial-join:\\ Compute all pairs $(\acr{id}(a_i), \acr{id}(b_j))$ with $\acr{mbr}(a_i) \cap \acr{mbr}(b_j) \ne \emptyset$\\
		\item \acr{id}-spatial-join:\\ Compute all pairs $(\acr{id}(a_i), \acr{id}(b_j))$ with $a_i \cap b_j \neq \emptyset$\\
